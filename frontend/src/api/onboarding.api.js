...
export async function signupOnboarding({
  ...
  const { data } = await request(`${DEMO_API_BASE}/api/onboarding/signup`, {
    method: 'POST',
    body: JSON.stringify({
      contactName,
      businessName,
...
export async function verifyOnboarding({ customerId }) {
...
export async function getOnboardingStatus(customerId) {
  const { data } = await request(`${DEMO_API_BASE}/api/onboarding/status/${customerId}`);
  return data;
}

// POST /api/onboarding/vip
// Saves VIP numbers. Best-effort — caller should catch and ignore errors.
export async function saveVipNumbers({ customerId, vipNumbers }) {
